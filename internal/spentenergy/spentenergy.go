...
func WalkingSpentCalories(steps int, weight, height float64, duration time.Duration) (float64, error) {
	if weight <= 0 || height <= 0 {
		return 0, fmt.Errorf("ошибка, вес или рост равны или меньше нуля")
	}
	if duration <= 0 {
		return 0, fmt.Errorf("ошибка, продолжительность активности равна или меньше нуля")
	}
...
func RunningSpentCalories(steps int, weight float64, duration time.Duration) (float64, error) {
	if weight <= 0 {
		return 0, fmt.Errorf("ошибка, вес меньше или равен нулю")
	}
	if duration <= 0 {
		return 0, fmt.Errorf("ошибка, продолжительность прогулки равна или меньше нуля")
	}
	meanSpeed := MeanSpeed(steps, duration)
